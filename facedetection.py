...
while 1: 
    ...
    for (x,y,w,h) in faces: 
        ...
        print(count)
        #cv2.imwrite(filename='saved_img'+str(count)+'.jpg', img=crop_img)
    ...
    if(count==7):
            ...
            if(emotion[-1]==6 and occurence_count[6]==4 and len(set(emotion))==2):
            ...
            print(emotion_dict[final_emotion])
            flag=input("If you want a song suggestion, press Y else N=")
            if(flag.upper()=="Y"):
            ...
            count=0
    if(len(emotion1)!=0):
        cv2.putText(img,emotion_dict[st.mode(emotion1)],(290,100),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),2)
